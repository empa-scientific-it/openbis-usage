...
class NewUserEventGatherer(override val api: IApplicationServerApi,
    ...
    override fun gather(token: String): List<InstanceEvent> {
        ...
        return result.objects.filter{ filterRegistrationDate(it, startDate, endDate) }.map{ it-> NewPersonEvent(DateUtils.toDateTime(it.registrationDate), it?.space?.code ?: "none", url)}
    }
}